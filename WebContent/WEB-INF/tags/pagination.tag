...
<ul class="pagination">
     		<c:choose>
	     		<c:when test="${currentPage != 1}">
	     			<li><a href="dashboard?page=${currentPage -1}&search=${search}&orderField=${orderField}">&laquo;</a></li>
	     		</c:when>
	    		<c:otherwise>
            ...
            <c:forEach begin="1" end="${countPages}" var="i">
                <li><a href="dashboard?page=${i}&search=${search}&orderField=${orderField}"> ${i} </<li></td>
            </c:forEach>
            <c:choose>
	     		<c:when test="${currentPage lt countPages}">
	     			<li><a href="dashboard?page=${currentPage +1}&search=${search}&orderField=${orderField}">&raquo;</a></li>
	     		</c:when>
	    		<c:otherwise>
